"""
Training script for fine-tuning Qwen 2.5 7B on retrieval task using LoRA/PEFT.

This script uses LoRA (Low-Rank Adaptation) for parameter-efficient fine-tuning,
which significantly reduces memory usage compared to full fine-tuning.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Fine-tune Qwen 2.5 7B on retrieval task using LoRA")
    ...
    parser.add_argument("--learning_rate", type=float, default=2e-4,
                        help="The initial learning rate for AdamW (higher for LoRA)")
...
def main():
    ...
    if args.use_lora:
        ...
        # Prepare model for kbit training (helps with memory)
