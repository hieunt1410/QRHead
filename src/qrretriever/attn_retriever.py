...
class AttnBasedRetriever:
    ...
    def __init__(
        ...
        config = self.setup_config(
            config_or_config_path, model_name_or_path, model_base_class
        )
        ...
        # init model etc
        if self.model_base_class.lower() in [
        ...
        elif self.model_base_class.lower() in ["qwen2.5-7b-instruct", "qwen2-0.5b-instruct"]:
    ...
    def setup_config(
        ...
        if config_or_config_path is not None:
            ...
            if model_base_class is not None:
                ...
                raise ValueError(
                    "model_name_or_path or model_base_class is required to use default config"
                )
        return config

    def get_content_span(
        self, prompt: str, char_offset_to_token_idx: dict, content: str
    ):
    ...
    def get_prompt(self, query: str, docs: List[Dict]):
        if self.model_base_class.lower() in [
            ...
            raise NotImplementedError(
                "Prompt prefix and suffix not defined for the model of {}.".format(
                    self.model_base_class
                )
            )
        ...
        llm_prompt += (
            self.prompt_separator
            + self.retrieval_instruction_late
            + self.prompt_separator
            + "Query:"
        )
    ...
    def compose_scoring_prompt(self, query: str, docs: List[Dict]) -> Tuple:
        ...
        prompt_tokenization_output = self.tokenizer(
            llm_prompt, return_offsets_mapping=True
        )
        ...
        for i, doc in enumerate(docs):
            ...
            start_idx, end_idx = self.get_content_span(
                llm_prompt, char_offset_to_token_idx, doc_content
            )
        ...
        query_start_idx, query_end_idx = self.get_content_span(
    ...
    def score_docs(self, query: str, docs: List[Dict]) -> Dict[str, Dict]:
        ...
        """
        prompt, tokenized_prompt, query_span, doc_spans = self.compose_scoring_prompt(
            query, docs
        )
        null_prompt, tokenized_null_prompt, null_query_span, _ = (
            self.compose_scoring_prompt(self.null_query, docs)
        )

        # up to query prompt the tokenized_prompt is the same
        assert (
            tokenized_null_prompt[: query_span[0]] == tokenized_prompt[: query_span[0]]
        )
        assert (
            query_span[0] == null_query_span[0]
        ), "Query start indices do not match between query and null query."

        # scoring with actual query
        per_token_scores, kv_cache = self.score_per_token_attention_to_query(
            prompt, query_span, None, 0
        )
        ...
        null_per_token_scores, _ = self.score_per_token_attention_to_query(
            null_prompt, null_query_span, kv_cache, start_idx
        )

        min_length = min(per_token_scores.shape[-1], null_per_token_scores.shape[-1])
        per_token_scores_CAL = (
            per_token_scores[:, :, :min_length]
            - null_per_token_scores[:, :, :min_length]
        )
        ...
        if self.attn_head_set == SPEC_HEAD_SET.FULL_SET:
            per_token_scores_CAL = per_token_scores_CAL.sum(
                0
            )  # sum per-token scores across layers
            per_token_scores_CAL = per_token_scores_CAL.sum(
                0
            )  # sum per-token scores across attention heads
        else:
            ...
            layers = indices[:, 0]
            heads = indices[:, 1]
            per_token_scores_CAL = per_token_scores_CAL[
                layers, heads
            ]  # Shape: (num_selected_heads, num_tokens)
        ...
        for i, doc_span in enumerate(doc_spans):
            curr_doc_per_tok_scores_CAL = per_token_scores_CAL[
                doc_span[0] : doc_span[1] + 1
            ]

            threshold = (
                curr_doc_per_tok_scores_CAL.mean()
                - 2 * curr_doc_per_tok_scores_CAL.std()
            )
            tok_mask = curr_doc_per_tok_scores_CAL > threshold

            per_doc_scores.append(
                (curr_doc_per_tok_scores_CAL * tok_mask).sum()
            )  # sum scores for each document

        assert len(per_doc_scores) == len(
            docs
        ), "Number of per-document scores does not match number of documents."
    ...
    def score_docs_per_head_for_detection(
        ...
        for i, doc_span in enumerate(doc_spans):
            ...
            threshold = curr_doc_per_tok_scores_CAL.mean(
                dim=-1
            ) - 2 * curr_doc_per_tok_scores_CAL.std(
                dim=-1
            ...
            tok_mask = curr_doc_per_tok_scores_CAL > threshold.unsqueeze(
                -1
            )  # shape (n_layers, n_heads, n_tok)
        ...
        assert len(per_doc_score_tensors) == len(
            docs
        ), "Number of per-document scores does not match number of documents."
    ...
    def score_per_token_attention_to_query(
        self, prompt, query_span, kv_cache=None, start_idx=0
    ):
        ...
        query_indices = list(
            range(query_span[0] - start_idx, query_span[1] - start_idx + 1)
        )
        ...
        for i in range(self.start_layer, self.end_layer + 1):
            attn_weights = (
                self._get_attn_weights(kv_cache.key_cache[i], kv_cache.query_cache[i])
                .to(self.device)
                .squeeze(0)
            )  ######## TODO: [:,:,:query_span[0]+1] OR [:,:,:query_span[1]+1] OR [:,:,:] ???
            attn_weights = attn_weights.mean(1)  # average over query tokens
            per_token_scores.append(attn_weights.squeeze(0))

        per_token_scores = torch.stack(
            per_token_scores, dim=0
    ...
    def _get_attn_weights(self, key_states, query_states):
        ...
        attn_lses = torch.logsumexp(
            attn_weights, dim=-1, keepdim=True
        )  # Log-sum-exp of attention weights for numerical stability in softmax.
        ...
        if config_or_config_path is not None:
            ...
            if model_base_class is not None:
                ...
                if model_base_class.lower() == "llama-3.1-8b-instruct":
                    config = load_config(
                        CONFIG_DIR / "Llama-3.1-8B-Instruct_full_head.yaml"
                    )
                elif model_base_class.lower() == "llama-3.1-70b-instruct":
                    config = load_config(
                        CONFIG_DIR / "Llama-3.1-70B-Instruct_full_head.yaml"
                    )
                elif model_base_class.lower() == "llama-3.2-3b-instruct":
                    config = load_config(
                        CONFIG_DIR / "Llama-3.2-3B-Instruct_full_head.yaml"
                    )
                elif model_base_class.lower() == "llama-3.2-1b-instruct":
                    config = load_config(
                        CONFIG_DIR / "Llama-3.2-1B-Instruct_full_head.yaml"
                    )
                elif model_base_class.lower() == "qwen2.5-7b-instruct":
                    config = load_config(
                        CONFIG_DIR / "Qwen2.5-7B-Instruct_full_head.yaml"
                    )
                elif model_base_class.lower() == "qwen2-0.5b-instruct":
                    config = load_config(
                        CONFIG_DIR / "Qwen2-0.5B-Instruct_full_head.yaml"
                    )
                else:
                    raise NotImplementedError(
                        f"Config inference for model_base_class {model_base_class} is not implemented."
                    )
            elif model_name_or_path is not None:
                ...
                elif "qwen2-0.5b-instruct" in model_name_or_path.lower():
                    config = load_config(
                        CONFIG_DIR / "Qwen2-0.5B-Instruct_full_head.yaml"
                    )
                else:
                    raise NotImplementedError(
                        f"Config inference for model_name_or_path {model_name_or_path} is not implemented."
                    )
            else:
                raise ValueError(
                    "model_name_or_path or model_base_class is required to use default config"
...
class QRRetriever(AttnBasedRetriever):
    ...
    def setup_config(
        ...
        if config_or_config_path is not None:
            ...
            print(
                "config_or_config_path is not provided. Use default config: LME for qr-head.",
                flush=True,
            )
            # infer from model information
            if model_base_class is not None:
                ...
                if model_base_class.lower() == "llama-3.1-8b-instruct":
                    config = load_config(
                        CONFIG_DIR / "Llama-3.1-8B-Instruct_qr_head_LME.yaml"
                    )
                elif model_base_class.lower() == "llama-3.1-70b-instruct":
                    config = load_config(
                        CONFIG_DIR / "Llama-3.1-70B-Instruct_qr_head_LME.yaml"
                    )
                elif model_base_class.lower() == "llama-3.2-3b-instruct":
                    config = load_config(
                        CONFIG_DIR / "Llama-3.2-3B-Instruct_qr_head_LME.yaml"
                    )
                elif model_base_class.lower() == "llama-3.2-1b-instruct":
                    config = load_config(
                        CONFIG_DIR / "Llama-3.2-1B-Instruct_qr_head_LME.yaml"
                    )
                elif model_base_class.lower() == "qwen2.5-7b-instruct":
                    config = load_config(
                        CONFIG_DIR / "Qwen2.5-7B-Instruct_qr_head_LME.yaml"
                    )
                elif model_base_class.lower() == "qwen2-0.5b-instruct":
                    config = load_config(
                        CONFIG_DIR / "Qwen2-0.5B-Instruct_qr_head_LME.yaml"
                    )
                else:
                    raise NotImplementedError(
                        f"Config inference for model_base_class {model_base_class} is not implemented."
